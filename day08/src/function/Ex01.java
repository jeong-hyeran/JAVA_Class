...
public class Ex01 {
	// 함수 정의 (어떤 형식으로 실행할지 결저아고, 함수의 이름과 형태를 지정한다.)
	// 정의하는 시점에서 실행되는 것은 아니다.
	static int add(int n1, int n2) {	// 인자를 매개변수에 받아서
		int answer = n1 + n2;			// 매개변수의 값을 이용하여 코드를 진행(매개변수는 변할 수 있다.)
		return answer;					// 마지막에 결과값을 반환한다.
	}									// 반환값은 함수 호출자리로 되돌아가게 되어 있따
	...
	public static void main(String[] args) {
		...
		// 대부분, 입력값이 돈재한다.
		// 일정한 코드를 수행하낟.
		// 배부분 출력값( 결과값, 반환값)이 존재하다.
		
		// 복잡한 단계로 일어나는 동작을 하난의 개념으로 묶어서 표현할 수 있게 된다.
		/// 입력값 x에 따라서 f를 통해서 y를 출력하라
