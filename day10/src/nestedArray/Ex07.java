...
public class Ex07 {
	...
	public static void main(String[] args) {
		...
		/// 선형구조(선으로 나타낼수있는 구조 (트리구조는 비선형 구조))
		...
		// 서로 다른 두 값의 크기 비교가 필요하다. /// 숫자열이라면 연산자를 이용해서 클래스는 함수를 이용하면 된다. ( A - B > 0) (compareTo)
		// 크기를 비교하여 내가 원하는 기준 (오름차순)에서 벗어나면 자리를 교체한다.
		// 두 변수의 값을 교환하기 위해서는 임시 변수가 하나 더 필요하다.
		// 두 값을 각각 지정하기 위해서, i번때 값과 j번째 값을 순환시켜야한다.
		
		for (int i = 0; i < arr.length-1; i++) {
			for (int j = i+1; j < arr.length; j++) {
				if(arr[i] - arr[j] > 0) {  /// comparㄷTo
					int tmp = arr[i];
					arr[i] = arr[j];
					arr[j] = tmp;
//					show(arr);
