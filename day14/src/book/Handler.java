...
//			if(arr[i] != null && arr[i].getName().contains(keyword)) {		//arr의 i인덱스에 비어있지 않거나, 키워드의 단어가 포함되어있다면
			if(arr[i] != null && (arr[i].getName().contains(keyword)||arr[i].getWriter().contains(keyword))) {		//arr의 i인덱스에 비어있지 않거나, 키워드의 단어가 포함되어있다면
...
//			if(arr[i] != null && arr[i].getName().contains(keyword)) { //arr의 i인덱스에 비어있지 않거나, 키워드의 단어가 포함되어있다면
	...
	public Book[] addCart (String name) {
...
//	public Book[] cartArray() {		//?? book의 타입의 목록을 한번에 보기 위한 함수
//		return cart[];
//	}
