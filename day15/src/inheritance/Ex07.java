...
class Actor extends Human { // 배우는 사람이다.
	...
	Actor(String name, String drame) {
		super(name);
		this.drama = drame;
	}
...
public class Ex07 {
	public static void main(String[] args) {
		...
		car.entrance(ac2);
							...
							////////// 클래스는 자료형이다!!!!!!!!
		Human t1 = ac1;		// 서브 클래스의 객체를 슈퍼클래스 타입으로 참조한다.(업캐스팅)
		...
		// 서브 클래스에서 재정의한 내용으로 실행된다.
		
		// 1) 함수호출이 가능한가?
		// 현재 슈퍼클래스에서 show() 메서드가 정의되어 있으므로 호출 가능
		/// 내용이 정의 되어있으면 호출 할 수 있다
		/// 실제 호출할때는 객체를 따라가지때문에 서브 클래스에서 덮어 썼기 때문에 오버라이딩 된 함수가 호출 된다.
