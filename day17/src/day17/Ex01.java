...
class Car{
	private Tire tire1;    ///타이어가 없으면 움직일 수 없는 자동차 (의존성을 가진다. 객체가 없으면 클래스가 움직일수없다.)
...
class SnowTire extends Tire{						// 상속에서 그 서브클래스는
	@Override										// 추상클래스의 추상메서드를 반드시 구현해야한다.
	public String toString() {
		return "스노우 타이어";
	}
}
...
public class Ex01 {
	public static void main(String[] args) {
		...
		car.setTire1(null);					// 자동차에 타이어를 해체한다.
		System.out.println(car);			// 자동차 상태 확인
		car.run();							// 자동차 테스트
		
		// 클래스 car가 Tire 타입의 객체를 필요로 할때 (의존성)
		// Tire의 서브 클래스(s,o,n)객체를 교체해서 연결해도
		// car의 작동에는 문제가 없어야 한다.
		/// 아예 없으면 문제가 되지만 어떤거란도 썼을때는 문제가 없어야 한다.
		/// car가 드라이브역할을 한다.
		//// 문법 적으로 이런경우에 추상화를 한다.
