...
/// 출력 함수 부분만 메인쪽에 만들기
	...
	public static void main(String[] args) {
		...
		Product tmp = null;   /// 추가할때 정보 객체로 ..저장하기위해?! 
		Product[] arr = null;
		int menu;
		int row;
		String name;
		...
		handler.insert(new IndustrialProduct("KF91 마스크 (검정)",1500,"korea"));
		handler.insert(new IndustrialProduct("KF91 마스크 (흰색)",1500,"korea"));
		handler.insert(new IndustrialProduct("에너자이저 건전지 AA x6",4800,"chlna"));
		...
		while (true) {
			...
			switch (menu) {
			case 1:			 // 전체목록(Handler에서 배열을 받아와서 반복문으로 출력)
				arr = handler.selectAll();
				for(Product p : arr)   /// 배열의 있는 것들을 하나하나 실행시킨다.
					if(p != null) {
						System.out.println(p);
					}
				break;
				
			case 2:			 // 추가 (입력받은 데이터를 객체화 해서 Handler에게 전달)
			...
			case 5:			 // 정렬 (상품 이름 순으로 정렬하기)/// 오름차순
//				System.out.print("정렬 기준 선택(1~4) : ");
//				menu = Integer.parseInt(sc.nextLine());
				
				
				arr = handler.sort(/*menu-1*/); /// 인자를 받는것과 위에 것들은 3번째 방식때문에
				for (int i = 0; i < arr.length; i++) {
					if (arr[i] != null) {
